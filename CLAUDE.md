...
- **Rider-facing sorties require qualified coverage at or under ratio** (R10): `request.rider_facing` is set at intake (`validateRequest()` refuses a showcase without riders), coverage is one row per (date, window, person), a person covering a window can't fly in it, and a coverer can't be released while the window's riders would be left uncovered. The desk's `ratio` and `min_per_window` live in `meta.rider_desk`, bounded 1–4, and there is no waiver.
...
Structured intake from free text (the form and its validation exist) · live multi-user state · lazy-load the seed JSON (initial chunk ~920 KB before gzip, ~120 KB gzipped; recharts already split) · CSV export of the integrity audit · rider incidents feeding RCCA, a rider comms log on the sortie, role-based notifications.
